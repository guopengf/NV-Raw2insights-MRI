...
class VascularAttentionAdapter(nn.Module):
    ...
    def __init__(
        ...
        if self.attention == "gate":
            self.net = nn.Sequential(
                nn.Conv2d(channels + self.prior_channels, hidden, kernel_size=1, bias=True),
                nn.GELU(),
            ...
            self.gate = nn.Sequential(
                nn.Conv2d(self.prior_channels, hidden, kernel_size=1, bias=True),
                nn.GELU(),
            ...
            self.v_proj = nn.Conv2d(channels, hidden, kernel_size=1, bias=False)
            self.out_proj = nn.Conv2d(hidden, channels, kernel_size=1, bias=True)
            if self.use_mask_bias:
                self.mask_bias = nn.Conv2d(self.prior_channels, self.num_heads, kernel_size=1, bias=True)
    ...
    def _prepare_prior(self, vessel_map: torch.Tensor, feature: torch.Tensor) -> torch.Tensor:
        ...
        if vessel_map.dim() != 4:
            raise ValueError(f"Expected vessel_map with shape [B,C,H,W], got {tuple(vessel_map.shape)}")
        ...
        if vessel_map.shape[1] == 1 and self.prior_channels > 1:
            return vessel_map.expand(-1, self.prior_channels, -1, -1)
        ...
        return torch.cat([vessel_map, vessel_map[:, -1:].expand(-1, pad, -1, -1)], dim=1)
    ...
    def forward(self, feature: torch.Tensor, vessel_map: torch.Tensor) -> torch.Tensor:
        ...
        k = k.view(b, self.num_heads, self.head_dim, hk * wk)
        v = v.view(b, self.num_heads, self.head_dim, hk * wk).transpose(-1, -2)

        attn = torch.matmul(q, k) * (self.head_dim ** -0.5)
        if self.use_mask_bias:
            bias = self.mask_bias(key_prior).view(b, self.num_heads, 1, hk * wk)
            attn = attn + self.mask_bias_scale.to(dtype=attn.dtype) * bias
        attn = torch.softmax(attn, dim=-1)
        delta = torch.matmul(attn, v)
        delta = delta.transpose(-1, -2).contiguous().view(b, self.hidden, h, w)
